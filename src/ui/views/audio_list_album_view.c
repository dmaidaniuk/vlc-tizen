...
static const void*
audio_list_album_item_get_media_item(list_view_item* p_item)
{
    return p_item->p_album_item;
}

static void
audio_list_album_item_set_media_item(list_view_item* p_item, void* p_data)
{
    album_item* p_media_item = (album_item*)p_data;
    p_item->p_album_item = p_media_item;
    ecore_main_loop_thread_safe_call_async((Ecore_Cb)elm_genlist_item_update, p_item->p_object_item);
}

static list_view_item*
audio_list_album_view_append_item(list_sys *p_sys, void* p_data)
{
...
audio_list_album_view_create(interface* p_intf, Evas_Object* p_parent, list_view_create_option opts)
{
    ...
    p_view->pf_append_item = &audio_list_album_view_append_item;
    p_view->pf_get_item = &audio_list_album_item_get_media_item;
    p_view->pf_set_item = &audio_list_album_item_set_media_item;

    if (opts & LIST_CREATE_MEDIA_CONTROLLER)
    {
        application* p_app = intf_get_application( p_intf );
        p_sys->p_ctrl = album_controller_create(p_app, p_view);
